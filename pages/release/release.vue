  ...
  <view id="window">
    <form @submit="onSubmit">
      ...
      <radio-group @change="radioChange">
        <radio value="public" checked="true" color="#000000" /><label
                ...
                success: function (ret) {
                  if (ret.size < 1045000) {
                    ...
                    wx.cloud.uploadFile({
                      cloudPath:
                        "tmp/" +
                        that.$options.methods.getRandomFileName(
                      ...
                      success: function (ret) {
                        wx.cloud.callFunction({
                          ...
                          complete: function (res) {
                            if (
                              !res.result.hasOwnProperty("errCode") ||
                              res.result.errCode != 52000
                            ) {
                              that["image"][index].status = "failed";
                              ...
                              wx.cloud.uploadFile({
                                cloudPath: that.$options.methods.getRandomFileName(
                                  that["image"][index].url
                                ),
